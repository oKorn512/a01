...
Context::Context(uint32_t w, uint32_t h, uint32_t sppx)
    : device(rtcNewDevice(0)), fbo(w, h, sppx), scene(device), cam(), handle(0), algo(0), window(0), quad(0) {
...
void Context::load_config(const fs::path& path) {
    const fs::path resolved_path = fs::exists(path) ? path : fs::path(GI_CONF_DIR) / path;
    json11::Json cfg = read_json_config(resolved_path.string().c_str());
    if (cfg.is_null())
        fprintf(stderr, "ERROR: Invalid config file specified: \"%s\"\n.", path.string().c_str());
    else
        from_json(cfg);
    if (algo) algo->read_config(cfg);
}

void Context::load_algorithm(const fs::path& path) {
    const fs::path resolved_path = fs::exists(path) ? path : fs::path(GI_BIN_DIR) / path;
    if (algo) delete algo;
    if (handle) close_module(handle);
    handle = open_module(resolved_path.string().c_str());
    Algorithm* (*create)() = (decltype(create)) resolve_symbol(handle, "create_algorithm");
    algo = create();
}
...
void Context::run() {
	if (!algo) {
		std::cerr << "Error: No algorithm module loaded!" << std::endl;
		return;
	}
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        time = now;
        glfwPollEvents();
        bool restart_rendering = keyboard_handler(window, *this, dt) || mouse_handler(window, *this, dt);
        ...
        if (ImGui::BeginMainMenuBar()) {
                        ...
                        if (ImGui::BeginMenu((std::string("Light #") + std::to_string(i)).c_str())) {
                            ...
                            if (dynamic_cast<AreaLight*>(scene.lights[i])) {
                                ...
                                if (ImGui::ColorEdit3("color", &l->mesh.mat->albedo_col.x))
                                    restart_rendering = true;
                                if (ImGui::DragFloat("power", &l->mesh.mat->emissive_strength, 1.f) && l->mesh.mat->emissive_strength > 0.f)
                                    restart_rendering = true;
                            ...
                            if (dynamic_cast<SkyLight*>(scene.lights[i])) {
                                ...
                                if (ImGui::DragFloat("intensity", &l->intensity, 0.1f))
                                    restart_rendering = true;
                                if (ImGui::Button("Extinguish")) {
                                    l->intensity = 0.f;
                                    restart_rendering = true;
            ...
            if (ImGui::BeginMenu("Algorithms (BETA)")) {
                std::vector<fs::path> algos;
                for(auto& p: fs::recursive_directory_iterator(GI_BIN_DIR))
                    if (p.path().string().find(".algo") != std::string::npos)
                        algos.push_back(p.path());
                ImGui::Text("Load algorithm module:");
                for (auto& p : algos) {
                    if (ImGui::Button(p.string().c_str())) {
                        abort = true; if (worker.joinable()) worker.join();
                        load_algorithm(p.string().c_str());
                        restart_rendering = true;
                    }
                }
                ImGui::EndMenu();
...
void Context::from_json(const json11::Json& cfg) {
    if (cfg.is_object()) {
        ...
        // update preview window if present
        if (window) {
