...
void trace_photons(const Context& context, uint32_t N, std::vector<PathVertex>& photons, bool scale_photon_power = true);

// collect N caustic photons
void trace_photons_caustics(const Context& context, uint32_t N, std::vector<PathVertex>& photons, bool scale_photon_power = true);
...
public:
    ...
    const glm::vec3 throughput;     ///< Path throughput up to this vertex
...
struct RandomWalkLight {
    UniformSampler1D light_sampler;
