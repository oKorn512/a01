...
void plot_histogram(const Distribution1D& dist) {
    ...
    // output
    static uint32_t i = 0;
    Texture::save_png(std::string("dist1D_") + std::to_string(i) + "_hits.png", w, h, buffer.data());
    Texture::save_png(std::string("dist1D_") + std::to_string(i++) + "_pdf.png", w, h, buffer_pdf.data());
}
