...
public:
    SkyLight();
    SkyLight(const fs::path& path, const Scene& scene, float intensity = 1.f);

    void load(const fs::path& path, const glm::vec3& scene_center, float scene_radius, float intensity = 1.f);
