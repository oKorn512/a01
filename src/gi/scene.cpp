...
void Scene::load_mesh(const fs::path& path) {
    ...
    const uint32_t ass_flags = aiProcess_Triangulate | aiProcess_GenNormals | aiProcess_FlipUVs |
        aiProcess_JoinIdenticalVertices | aiProcess_RemoveRedundantMaterials | aiProcess_OptimizeMeshes;
...
void Scene::load_sky(const fs::path& path, float intensity) {
    const fs::path resolved_path = fs::exists(path) ? path : fs::path(GI_DATA_DIR) / path;
