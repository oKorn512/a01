...
public:
    ...
    glm::vec3 N;                    ///< World space shading normal
     ...
     */
    inline glm::vec3 albedo() const { assert(mat); return mat->albedo(TC); }
     ...
     */
    inline float roughness() const { assert(mat); return mat->roughness(TC); }
    ...
    /**
     * @brief Fetch surface emission
     *
     * @return emitted light, either from static color or texture
     */
    inline glm::vec3 emission() const { assert(mat); return mat->emissive(TC); }
     ...
     */
    inline bool is_type(const BRDFType& type) const { assert(mat); return mat->brdf->is_type(type); }
     ...
     */
    glm::vec3 to_tangent(const glm::vec3& world_dir) const { assert(valid); return world_to_tangent(N, world_dir); }
     ...
     */
    glm::vec3 to_world(const glm::vec3& tangent_dir) const { assert(valid); return align(N, tangent_dir); }
};
