...
Texture::Texture(const fs::path& path, bool sRGB) : Texture() {
    load(path, sRGB);
}
...
void Texture::load(const fs::path& path, bool sRGB) {
    src_path = path;
...
void Texture::load_alpha(const fs::path& path) {
    src_path = path;
    // load image from disk
...
void Texture::save_png(const fs::path& path) const {
    Texture::save_png(path, w, h, data());
}

void Texture::save_jpg(const fs::path& path) const {
    Texture::save_jpg(path, w, h, data());
}

void Texture::save_png(const fs::path& path, size_t w, size_t h, const glm::vec3* rgb, bool flip) {
...
void Texture::save_jpg(const fs::path& path, size_t w, size_t h, const glm::vec3* rgb, bool flip) {
    stbi_flip_vertically_on_write(flip ? 1 : 0);
    std::vector<uint8_t> pixels(w * h * 3u);
    for (size_t y = 0; y < h; ++y)
        for (size_t x = 0; x < w; ++x)
