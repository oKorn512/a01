...
public:
    ...
    Texture(const fs::path& path, bool sRGB = true);
    ...
    void load(const fs::path& path, bool sRGB = true);
    // load alpha channel of texture from disk
    void load_alpha(const fs::path& path);
    ...
    inline glm::vec3 operator()(size_t x, size_t y) const { return fetch(glm::uvec2(x, y)); }
    ...
    inline fs::path path() const { return src_path; }

    // save as PNG / JPG
    void save_png(const fs::path& path) const;
    void save_jpg(const fs::path& path) const;

    // generalization for writing rgb data to disk as png or jpg
    static void save_png(const fs::path& path, size_t w, size_t h, const glm::vec3* rgb, bool flip = true);
    static void save_jpg(const fs::path& path, size_t w, size_t h, const glm::vec3* rgb, bool flip = true);

    // data
    size_t w;                       ///< Texture width
    size_t h;                       ///< Texture height
    std::vector<glm::vec3> texels;  ///< Raw texel data
    fs::path src_path;              ///< Filename, if loaded from disk
